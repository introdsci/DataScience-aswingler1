...
Switch "Representative" to "house" and "Senator" to "senate".
...
avg_pct_vote_by_gen <- ggplot(averages, aes(x=generation, y=agree_pct)) + stat_summary(fun.y="mean", geom="bar", color="green")
avg_pct_vote_by_gen
```
...
Models
```{r}
#model <- lm(data = politician, formula = generation ~ generation + num_votes)
